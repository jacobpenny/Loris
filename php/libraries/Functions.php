<?php
/**
 * This file defines functions to be used by the LORIS Parser (PHP)
 *
 * PHP Version 7
 *
 * @category Main
...
/**
 * Functions can be added to the array returned in getFunctions()
 * and syntax can be defined in getBinary() or getUnary()
 *
 * @category Main
 * @package  Loris
...
/**
 * Recursive factorial helper for integers
 *
 * @param integer $n value to recursively operate on
 *
 * @return integer $n!
 */
...
/**
 * Recursive factorial helper for floats
 *
 * @param float $n value to recursively operate on
 *
 * @return float $n!
 */
...
/**
 * Returns array of functions to be used by Evaluator
 *
 * @return array
 */
    ...
    return array(
            ...
            '_fact'        => function ($a) {
                if ($a >=0 && $a%1 == 0) {
                    ...
                    throw "Factorial for a number not divisible by 0.5"
                          ." or greater than 0 is not supported.";
                }
            },
            ...
            '_datediff'    => function ($date1, $date2, $units,
                $returnSigned=false
) {
                ...
                if ($units === 'y' || $units === 'Y') {
                    $res = ($interval->format("%y")+
                            $interval->format("%m")/12+
                            $interval->format("%d")/365);
                } else if ($units === 'm' || $units === 'M') {
                    $res = ($interval->format("%y")*12+
                            $interval->format("%m")+
                            $interval->format("%d")/30.44);
                } else if ($units === 'd' || $units === 'D') {
                    $res = ($interval->format("%y")*365+
                            $interval->format("%m")*30.44+
                            $interval->format("%d"));
                } else {
                    return 0;
                }
...
/**
 * Returns php function based on mathematical syntax
 *
 * @return function
 */
function getBinary()
